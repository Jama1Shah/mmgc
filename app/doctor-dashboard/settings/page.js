...
export default function DoctorSettings() {
    ...
    useEffect(() => {
        document.title = "Doctor Settings - MMGC";
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (data?.name) {
                        ...
                        setSpecialization(data.dept || "Specialist");
                    }
                }
            } catch (err) {
        ...
        <div className="min-h-screen bg-slate-50 flex relative">
            ...
            <main className="flex-1 w-full flex flex-col">
                <header className="bg-white border-b border-slate-200 px-4 md:px-10 py-6 sticky top-0 z-40 flex justify-between items-center">
                    ...
                    {/* Doctor Context Identity Info Display */}
                    <div className="flex items-center space-x-3 border-l pl-4 border-slate-200">
                        <div className="text-right hidden sm:block">
                            <p className="text-sm font-bold text-slate-800 leading-none">
                                {userName ? `Dr. ${userName}` : "Syncing..."}
                            </p>
                            <p className="text-xs text-slate-500 mt-1">{specialization}</p>
                        </div>
                    </div>
                </header>
